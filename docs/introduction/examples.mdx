...
In this example the the graph maps the x and y movement of the collective control. The collective position are is via Iotes over a public mqtt broker and received by the graph, which subscribes via Iotes
...
simpler access to Iotes for use in React and is recommended for use is [react projects](/docs/plugins/react-hooks)


```jsx
import { createIotes } from 'iotes'

// This topology map creates one device called 'COLLECTIVE' which subscribes to the public 
...
export const Example = () => {
  ...
  // Reference to the iotes instance
  const iotes = useRef(null)

  // References to track susription state
